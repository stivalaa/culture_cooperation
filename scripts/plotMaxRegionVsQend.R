...
# Plot q on x axis and max cluster size on y axis for results
...
# Usage:
#
# Rscript plotMaxRegionVsQend.R data.csv outputprefix
#
#
# E.g. Rscript plotMaxRegionVsQend.R results.csv varynoise-combined
#
...
# ADS April 2016
#
# $Id: plotMaxRegionVsQend.R 573 2016-05-06 04:26:21Z stivalaa $
...
if (length(commandArgs(trailingOnly=TRUE)) != 2) {
    cat("Usage: Rscript plotMaxRegionVsQend.R results.csv outputprefix\n")
    quit(save='no')
}
...
for (i in 1:length(responses)) {
  ...
  p <- ggplot(Dst, aes(x = q, y = value.mean,
...
##   }
  ...
  p <- p + scale_shape_manual('Noise rate',  labels = my_scientific_10, values=1:nlevels(as.factor(Dst$noise_rate))) # needed for more than 6 shapes
