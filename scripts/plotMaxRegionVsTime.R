...
# Usage:
#
# Rscript plotMaxRegionVstimedata.csv outputprefix
#
#
# E.g. Rscript plotMaxRegionVsTimeresults.csv varynoise-combined
#
...
pool_mult_value <-  0.6  # uses only one value of pool multiplier (MPCR)
...
orig_experiment <- orig_experiment[which(orig_experiment$pool_multiplier == pool_mult_value),]
...
#  experiment$m <- as.factor(experiment$m)
  experiment$noise_rate <- as.factor(experiment$noise_rate)
  if (constant_mpcr) {
...
#  p <- p + annotation_logticks(sides="b")
  ...
  p <- p + facet_grid(noise_rate ~ ., labeller=bla)
  p <- p + ggtitle (bquote(list(F == .(Fvalue), L == .(m_list[mi]),  plain(MPCR) == .(Dst$mpcr), plain(radius) == .(Dst$radius))))
